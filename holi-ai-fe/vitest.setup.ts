...
const mockCapacitor = { schedule: vi.fn(), getPending: vi.fn().mockResolvedValue({ notifications: [] }), cancel: vi.fn(), checkPermissions: vi.fn().mockResolvedValue({ display: 'granted' }), requestPermissions: vi.fn().mockResolvedValue({ display: 'granted' }), addListener: vi.fn().mockResolvedValue({ remove: vi.fn() }) };
