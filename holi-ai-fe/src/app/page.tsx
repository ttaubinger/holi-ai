...
const loadSettings = (s: any) => {
  ...
  s.setGroqModel(localStorage.getItem('GROQ_MODEL') || 'openai/gpt-oss-120b');
  ...
  s.setUnitSystem(localStorage.getItem('UNIT_SYSTEM') || 'Metric');
};

const syncConfigToBackend = async (s: any) => {
  try {
    const config = { GARMIN_CLIENT_ID: s.deviceClientId, GARMIN_CLIENT_SECRET: s.deviceClientSecret, GROQ_KEY: s.groqKey, GROQ_MODEL: s.groqModel, RAG_THRESHOLD: s.ragThreshold, DEBUG_MODE: s.debugMode, UNIT_SYSTEM: s.unitSystem };
...
const persistLocalKeys = (s: any) => {
  ...
  localStorage.setItem('UNIT_SYSTEM', s.unitSystem);
};
...
const ConfirmModal = ({ title, text, onConfirm, onCancel, dict }: any) => (
  <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, background: 'rgba(0,0,0,0.6)', zIndex: 4000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '1rem' }} onClick={onCancel}>
...
const TOOLTIPS = {
  ...
  groqModel: "REQUIRED: The specific AI model you want to run on Groq. The default is 'openai/gpt-oss-120b'. Check groq.com/pricing for other options.",
...
const PersonalityOption = ({ id, label, selectedId, setSelectedId }: any) => (
  <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
    <input type="radio" name="personality" value={id} checked={selectedId === id} onChange={() => setSelectedId(id)} />
...
const PersonalityOptions = ({ selectedId, setSelectedId, dict }: any) => (
  <>
    <PersonalityOption id="off" label={dict.personalityOff} selectedId={selectedId} setSelectedId={setSelectedId} />
    {PREDEFINED_PROMPTS.map(p => <PersonalityOption key={p.id} id={p.id} label={getLabel(p, dict)} selectedId={selectedId} setSelectedId={setSelectedId} />)}
    <PersonalityOption id="custom" label={dict.personalityCustom} selectedId={selectedId} setSelectedId={setSelectedId} />
  </>
);
...
const SettingsFormUnits = ({ s, dict }: any) => (
  <div style={{ marginTop: '1.5rem' }}>
    <h3 style={{ marginBottom: '0.75rem', fontSize: '1.1rem' }}>{dict.unitSystem}</h3>
    <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
      <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
        <input type="radio" name="unitSystem" value="Metric" checked={s.unitSystem === 'Metric'} onChange={() => s.setUnitSystem('Metric')} />
        <span style={{ fontSize: '1rem' }}>{dict.metricSystem}</span>
      </label>
      <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer' }}>
        <input type="radio" name="unitSystem" value="Imperial" checked={s.unitSystem === 'Imperial'} onChange={() => s.setUnitSystem('Imperial')} />
        <span style={{ fontSize: '1rem' }}>{dict.imperialSystem}</span>
      </label>
    </div>
  </div>
);

const SettingsFormPersonality = ({ personality, s, dict }: any) => (
  <>
    ...
    {personality.isLoading ? (
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.75rem', marginBottom: '1.5rem' }}>
        ...
        <SettingsFormUnits s={s} dict={dict} />
      </div>
    )}
  </>
);
...
const SettingsFormContent = ({ s, setInfo, setWiping, isLocalDb, personality, handleSave, handleWipe, wiping, info, dict }: any) => (
  <div style={{ padding: '1.5rem', flex: 1, overflowY: 'auto' }}>
    ...
    <SettingsFormPersonality personality={personality} s={s} dict={dict} />
...
const useSettingsState = () => {
  ...
  return { sbConnUrl, setSbConnUrl, neonUrl, setNeonUrl, groqKey, setGroqKey, groqModel, setGroqModel, ragThreshold, setRagThreshold, debugMode, setDebugMode, appleHealth, setAppleHealth, deviceClientId, setDeviceClientId, deviceClientSecret, setDeviceClientSecret, unitSystem, setUnitSystem };
};
...
const ChatTable = ({ node: _node, ...props }: any) => (
  <div className="markdown-table-wrapper" style={{ overflowX: 'auto', WebkitOverflowScrolling: 'touch', width: '100%', marginBottom: '1rem' }}>
    <table className="markdown-table" style={{ width: '100%' }} {...props} />
  </div>
);
...
const ChatInputActive = ({ input, onIn, onKey, submit, dict, isProcessing }: any) => (
  <div style={{ display: 'flex', flexDirection: 'column' }}>
    <form onSubmit={submit} style={{ display: 'flex', flexDirection: 'column', padding: '1rem', borderTop: '1px solid var(--border-color)' }}>
...
const getProgressBarLabel = (aiStatus: string) => {
  if (!aiStatus) return 'Processing...';
  const s = aiStatus.toLowerCase();
  if (s.includes('routine')) return 'Generating routines...';
  if (s.includes('plan')) return 'Generating plan...';
  if (s.includes('capacity') || s.includes('resume') || s.includes('limit')) return 'Waiting for AI capacity...';
  return 'Generating...';
};

const ProgressBar = ({ progress, aiStatus }: any) => {
  if (typeof progress !== 'number') return null;
  return (
    <div style={{ width: '100%', paddingTop: '1rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '0.4rem', padding: '0 1rem', fontSize: '0.85rem', color: 'var(--text-primary)', fontWeight: 'bold', boxSizing: 'border-box' }}>
        <span>{getProgressBarLabel(aiStatus)}</span>
        <span>{progress}%</span>
      </div>
...
const ChatViewMessages = ({ ref, onScroll, hasMore, loadMore, isLoadingMore, state, retryMessage, lastUser, dict }: any) => (
  <div ref={ref} onScroll={onScroll} className="no-scrollbar" style={{ flex: 1, overflowY: 'auto', padding: '1rem', display: 'flex', flexDirection: 'column' }}>
    ...
    {state.isThinking && !state.isDelayed && state.aiStatus !== 'HIDDEN' && <div style={{ color: 'var(--text-secondary)', padding: '1rem' }}>{state.aiStatus || dict.coachAnalyzing}</div>}
...
const ChatView = ({ state, sendMessage, retryMessage, loadMore, hasMore, isLoadingMore, dict, chatInput, setChatInput }: any) => {
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <ProgressBar progress={state.progress} aiStatus={state.aiStatus} />
      <ChatInput input={chatInput} setInput={setChatInput} send={sendMessage} dict={dict} isProcessing={isProcessing} isStuck={isStuck} onRetry={() => retryMessage(lastUser?.content)} />
    </div>
  );
};
...
const getCronTimeMinutes = (expr?: string) => {
  if (!expr) return Infinity;
  const parts = expr.trim().split(/\s+/);
  if (parts.length >= 2) {
    const m = parseInt(parts[0] as string, 10);
    const h = parseInt(parts[1] as string, 10);
    if (!isNaN(m) && !isNaN(h)) return h * 60 + m;
  }
  return Infinity;
};

const sortCrons = (crons: Cron[], sortOrder: 'asc' | 'desc') => {
  return (crons || []).slice().sort((a: Cron, b: Cron) => {
    const timeA = getCronTimeMinutes(a.cron_expression);
    const timeB = getCronTimeMinutes(b.cron_expression);
    if (timeA === Infinity && timeB === Infinity) return (a.title || '').localeCompare(b.title || '');
...
const useActiveRoutinesState = (crons: any, injectedFilter: string, setInjectedFilter: any, reads: string[], markRead: any) => {
  const [selectedCron, setSelectedCronState] = useState<Cron | null>(null);
...
const CategoryTabs = ({ categories, filter, setFilter, setCreatingCron }: any) => (
  <div style={{ display: 'flex', gap: '0.5rem', overflowX: 'auto', marginBottom: '1rem', paddingBottom: '0.5rem', alignItems: 'center' }}>
    ...
    <button onClick={() => setCreatingCron(true)} style={{ background: 'var(--accent-color)', color: '#fff', border: 'none', borderRadius: '50%', minWidth: '30px', height: '30px', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', fontSize: '1.2rem', paddingBottom: '3px', marginLeft: 'auto' }}>+</button>
  </div>
);
...
const SortToggle = ({ sortOrder, setSortOrder }: any) => (
  <button onClick={() => setSortOrder(sortOrder === 'asc' ? 'desc' : 'asc')} style={{ background: 'var(--panel-bg)', color: 'var(--text-primary)', border: '1px solid var(--border-color)', borderRadius: '16px', padding: '0.3rem 0.8rem', cursor: 'pointer', fontSize: '0.8rem', display: 'flex', alignItems: 'center', gap: '0.3rem' }}>
    {sortOrder === 'asc' ? '↓ Time' : '↑ Time'}
  </button>
);
  ...
  return (
    <div style={{ marginTop: '2rem' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1rem' }}>
        <h3 style={{ margin: 0, color: 'var(--text-secondary)' }}>{dict.routines}</h3>
        <SortToggle sortOrder={s.sortOrder} setSortOrder={s.setSortOrder} />
      </div>
...
const ChatMessageItem = ({ t }: any) => {
  ...
  return (
    <div id={`trace-${t.id}`} className="card" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: isUser ? '4px solid var(--accent-color)' : '4px solid var(--primary-color)', background: 'rgba(30, 30, 40, 0.3)' }}>
      <div>
...
const TraceLLMItem = ({ t, setSelectedTrace, isRag }: any) => {
  ...
  return (
    <div id={`trace-${t.id}`} className="card" style={{ cursor: 'pointer', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: s.border, background: s.bg }} onClick={() => setSelectedTrace(t)}>
      <div>
...
const LoadMoreButton = ({ hasMore, isLoadingMore, loadMoreTraces }: any) => {
  if (!hasMore) return null;
  return (
    <button
      className="apple-button"
      style={{ width: '100%', padding: '0.75rem', opacity: isLoadingMore ? 0.6 : 1 }}
...
const TraceList = ({ traces, isLoading, setSelectedTrace, hasMore, isLoadingMore, loadMoreTraces }: any) => (
  <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
    ...
    <LoadMoreButton hasMore={hasMore} isLoadingMore={isLoadingMore} loadMoreTraces={loadMoreTraces} />
  </div>
);
...
const DebugHeaderButtons = ({ onShowQueue, fetchTraces, isLoading }: any) => (
...
const DebugViewHeader = ({ fetchTraces, isLoading, onShowQueue }: any) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
    ...
    <DebugHeaderButtons fetchTraces={fetchTraces} isLoading={isLoading} onShowQueue={onShowQueue} />
  </div>
);

/* eslint-disable max-lines-per-function */
const DebugView = () => {
  const { traces, isLoading, isLoadingMore, hasMore, error, fetchTraces, loadMoreTraces, queueItems, isQueueLoading, queueError, fetchQueue } = useDebug();
  ...
  const handleTraceSelect = (t: LlmTrace | null) => {
    if (t) {
  ...
  useEffect(() => {
    if (!selectedTrace && lastSelectedIdRef.current) {
      setTimeout(() => {
        const el = document.getElementById(`trace-${lastSelectedIdRef.current}`);
        if (el) {
  ...
  return (
    <div ref={scrollRef} style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', background: 'var(--bg-color)' }}>
      ...
      {selectedTrace ? <TraceDetails selectedTrace={selectedTrace} setSelectedTrace={handleTraceSelect} /> : <TraceList traces={traces} isLoading={isLoading} setSelectedTrace={handleTraceSelect} hasMore={hasMore} isLoadingMore={isLoadingMore} loadMoreTraces={loadMoreTraces} />}
    </div>
  );
};
...
const BiometricsLogForm = ({ dict, onSubmit, onClose }: any) => {
  ...
  return (
    <div className="biometrics-form-container" style={{ maxWidth: '600px', margin: '0 auto', padding: '1.5rem' }}>
      <ModalHeader title={dict.logBiometricsData} onClose={onClose} />
...
const ModalHeader = ({ title, onClose }: any) => (
  <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.5rem', paddingTop: 'max(1rem, env(safe-area-inset-top))' }}>
    <button onClick={onClose} style={{ background: 'var(--panel-bg)', borderRadius: '50%', border: '1px solid var(--border-color)', color: 'var(--text-primary)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', width: '40px', height: '40px', flexShrink: 0 }}>
      <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><polyline points="15 18 9 12 15 6"></polyline></svg>
    </button>
    <h2 style={{ margin: 0, color: 'var(--accent-color)', fontSize: '1.5rem' }}>{title}</h2>
  </div>
);
  ...
  <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center' }}>
    {!isBoolean && (
      <div style={{ display: 'flex', alignItems: 'center', gap: '0.25rem' }}>
        <input type={c.log_type === 'number' ? 'number' : 'text'} className="apple-input" value={(data[c.cron_id] as string) || ''} onChange={(e) => setData({ ...data, [c.cron_id]: e.target.value })} style={{ width: '80px', padding: '0.5rem 0.75rem', fontSize: '0.9rem', textAlign: 'center' }} />
        {c.log_unit && <span style={{ fontSize: '0.85rem', color: 'var(--text-secondary)' }}>{c.log_unit}</span>}
      </div>
    )}
    <div style={{ display: 'flex', gap: '0.5rem' }}>
...
const ActivitiesView = ({ dict, crons, onClose }: any) => {
  ...
  return (
    <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'var(--bg-color)', zIndex: 3000, overflowY: 'auto' }}>
      <div className="biometrics-form-container" style={{ maxWidth: '600px', margin: '0 auto', padding: '1.5rem' }}>
        <ModalHeader title={dict.logActivities} onClose={onClose} />
...
const EditRoutineModalField = ({ label, val, set, multi }: any) => (
  <label style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem', fontSize: '0.9rem', color: 'var(--text-secondary)' }}>
    {label}
    {multi ? <textarea value={val || ''} onChange={e => set(e.target.value)} className="apple-input" rows={4} /> : <input type="text" value={val || ''} onChange={e => set(e.target.value)} className="apple-input" />}
  </label>
);
...
const useReadRoutines = () => {
  const [reads, setReads] = useState<string[]>([]);
  useEffect(() => {
    const stored = localStorage.getItem(`read_routines_${new Date().toISOString().split('T')[0]}`);
    if (stored) setReads(JSON.parse(stored));
  }, []);
  const markRead = (cron_id: string) => {
    setReads((prev: string[]) => {
      if (prev.includes(cron_id)) return prev;
      const next = [...prev, cron_id];
      localStorage.setItem(`read_routines_${new Date().toISOString().split('T')[0]}`, JSON.stringify(next));
      return next;
    });
  };
...
const useAppContentState = () => {
  ...
  const cronsState = useCrons(init.configured, init.tab, `${chat.state.messages.length}-${chat.state.progress === 100}`);
  const modsState = useModules(init.configured, init.tab, `${chat.state.messages.length}-${chat.state.progress === 100}`);
  const { reads, markRead } = useReadRoutines();
  const [menu, setMenu] = useState({ open: false, biometrics: false, activities: false, reports: false, grouped_routines: false });
...
const AppContentModals = ({ s, dict }: any) => {
  const wrapClose = (action: () => void) => () => {
    action();
    if (s.exitOnClose) { s.setExitOnClose(false); App.exitApp(); }
  };
  return (
    <>
...
const useAppNotifications = (s: any) => {
  useNotifications(s.cronsState.crons, s.cronsState.deleteCron, (route, extra, appWasClosed) => {
    if (appWasClosed) s.setExitOnClose(true);
    if (route === 'activities') s.setMenu((prev: any) => ({ ...prev, open: false, activities: true }));
...
const AppContent = () => {
  ...
  return (
    <>
      {s.init.configured && <button className="hamburger-btn" style={{ zIndex: 2002, position: 'fixed' }} onClick={() => s.setMenu({ ...s.menu, open: !s.menu.open })}>{s.menu.open ? '✕' : '☰'}</button>}
