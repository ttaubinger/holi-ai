...
const DebugHeaderButtons = ({ onShowQueue, triggerTestPlan, setActiveJobId, fetchTraces, isLoading }: any) => (
  <div style={{ display: 'flex', gap: '0.5rem' }}>
    ...
    <button className="apple-button secondary" style={{ width: 'auto', padding: '0.5rem 1rem', background: 'var(--accent-color)', color: 'white', border: 'none' }} onClick={() => triggerTestPlan(setActiveJobId)} disabled={isLoading}>
      Test Plan Gen
    </button>
...
const DebugViewHeader = ({ fetchTraces, isLoading, onShowQueue, triggerTestPlan, setActiveJobId }: any) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '1.5rem' }}>
    <h2 style={{ margin: 0 }}>LLM Traces</h2>
    <DebugHeaderButtons fetchTraces={fetchTraces} isLoading={isLoading} onShowQueue={onShowQueue} triggerTestPlan={triggerTestPlan} setActiveJobId={setActiveJobId} />
  </div>
);

const DebugView = ({ setActiveJobId }: any) => {
  const { traces, isLoading, isLoadingMore, hasMore, error, fetchTraces, loadMoreTraces, queueItems, isQueueLoading, queueError, fetchQueue, triggerTestPlan } = useDebug();
  ...
  return (
    <div style={{ flex: 1, overflowY: 'auto', padding: '1.5rem', background: 'var(--bg-color)' }}>
      ...
      <DebugViewHeader fetchTraces={fetchTraces} isLoading={isLoading} onShowQueue={() => { setShowQueue(true); fetchQueue(); }} triggerTestPlan={triggerTestPlan} setActiveJobId={setActiveJobId} />
      {error && <div style={{ color: 'var(--error-color, #ff4444)', marginBottom: '1rem' }}>{error}</div>}
      {selectedTrace ? <TraceDetails selectedTrace={selectedTrace} setSelectedTrace={setSelectedTrace} /> : <TraceList traces={traces} isLoading={isLoading} setSelectedTrace={setSelectedTrace} hasMore={hasMore} isLoadingMore={isLoadingMore} loadMoreTraces={loadMoreTraces} />}
    </div>
  );
};

const AppTabs = ({ configured, tab, setTab, state, sendMessage, retryMessage, loadMore, hasMore, isLoadingMore, setConfigured, dict, crons, deleteCron, toggleCron, modules, deleteModule, clearChat, chatInput, setChatInput, setActiveJobId }: any) => (
  <div style={{ paddingTop: '3rem', flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
    ...
    {configured && tab === 'debug' && <DebugView setActiveJobId={setActiveJobId} />}
...
const useAppContentState = () => {
  ...
  const syncTrigger = `${chat.state.messages.length}-${chat.state.progress === 100}`;
  const cronsState = useCrons(init.configured, init.tab, syncTrigger);
  const modsState = useModules(init.configured, init.tab, syncTrigger);
