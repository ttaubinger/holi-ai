...
const buildNativeNotification = (c: Cron): LocalNotificationSchema | null => {
  ...
  return {
    ...
    group: c.cron_id,
    extra: { route: c.requires_logging ? 'activities' : 'routine_detail', cron_id: c.cron_id } 
  };
};
...
const useNativeScheduling = (crons: Cron[], intervalRef: any, onDeepLink?: (route: string, extra?: any) => void) => {
  useEffect(() => {
    ...
    const listener = LocalNotifications.addListener('localNotificationActionPerformed', (action) => {
      ...
      if (extra?.route && onDeepLink) onDeepLink(extra.route, extra);
    });
    return () => {
      if (intervalRef.current) clearInterval(intervalRef.current);
...
export const useNotifications = (crons: Cron[], deleteCron: (id: string) => void, onDeepLink?: (route: string, extra?: any) => void) => {
