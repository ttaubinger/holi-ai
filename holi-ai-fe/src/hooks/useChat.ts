...
const fetchPendingJob = async (userId: string) => {
  const url = `${getApiUrl()}/chat/pending?userId=${userId}`;
  const raw = await doFetch(url);
  return raw.job;
};
...
const getInitialState = (): ChatState => {
  if (typeof window === 'undefined') return { messages: [], isThinking: false, aiStatus: null, progress: null, error: null };
  const cached = localStorage.getItem('chat-cache');
  return { messages: cached ? JSON.parse(cached) : [], isThinking: false, aiStatus: null, progress: null, error: null };
};

const handleJobFailed = (jobId: string, result: any, setState: any, setActive: any, _setOffset: any) => {
  let aiReply = '';
  ...
  if (aiReply) {
    ...
    setState((p: ChatState) => ({ ...p, isThinking: false, aiStatus: null, isDelayed: false, progress: null, messages: [...p.messages, msg] }));
    setActive(null);
  } else {
    setState((p: ChatState) => ({ ...p, isThinking: false, aiStatus: null, isDelayed: false, progress: null, error: result?.error || 'Job failed' }));
  }
  setActive(null);
};
...
const useInitHistory = (userId: string, configured: boolean, setState: any, setOffset: any, setHasMore: any, setActiveJobId: any) => {
  useEffect(() => {
    let isMounted = true;
    if (!configured) return;
    
    fetchPendingJob(userId).then(job => {
      if (isMounted && job) setActiveJobId(job.id);
    }).catch(console.error);
  ...
  }, [userId, configured, setState, setOffset, setHasMore, setActiveJobId]);
};

const handleAppResume = async (isActive: boolean, configured: boolean, userId: string, setState: any, setOffset: any, setActiveJobId: any) => {
  if (!isActive || !configured) return;
  try {
    const job = await fetchPendingJob(userId);
...
const useAppResume = (userId: string, configured: boolean, setState: any, setOffset: any, setActiveJobId: any) => {
  useEffect(() => {
    let handle: any = null;
    if (typeof window !== 'undefined') {
      App.addListener('appStateChange', (s: any) => handleAppResume(s.isActive, configured, userId, setState, setOffset, setActiveJobId)).then(h => { handle = h; });
    }
    return () => { if (handle) handle.remove(); };
  }, [userId, configured, setState, setOffset, setActiveJobId]);
};
...
const handleJobDelayed = async (result: any, setState: any, setOffset: any, userId: string, runNext: (w: number) => void) => {
  try {
    ...
    const hist = applyHistory(data);
    if (hist.length > 0) {
      setState((p: ChatState) => ({ ...p, isThinking: true, aiStatus: result?.system_message || p.aiStatus, isDelayed: true, progress: result?.progress ?? p.progress, messages: hist }));
      setOffset(data.messages.length);
    }
  } catch(_e) {}
...
const updatePendingState = (result: any, lang: string, status: string) => (p: ChatState) => {
  ...
  if (result?.system_message) {
    txt = result.system_message;
  } else if (status === 'pending') {
    txt = dict?.waitingForServer || 'Waiting for server capacity...';
  }
  return { ...p, isThinking: true, aiStatus: txt, isDelayed: false, progress: result?.progress ?? p.progress ?? null };
};
...
export const useChat = (userId: string, lang: string, configured: boolean) => {
  ...
  useInitHistory(userId, configured, s.setState, s.setOffset, s.setHasMore, s.setActiveJobId);
  useAppResume(userId, configured, s.setState, s.setOffset, s.setActiveJobId);
  ...
  return { state: s.state, sendMessage, retryMessage: sendNewMessage, loadMore, hasMore: s.hasMore, isLoadingMore: s.isLoadingMore, clearChat, chatInput: s.chatInput, setChatInput: s.setChatInput, setActiveJobId: s.setActiveJobId };
};
