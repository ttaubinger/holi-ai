...
const doTriggerTestPlan = async (keys: any) => {
  const url = `${getApiUrl()}/debug/test-plan`;
  const res = await fetch(url, { method: 'POST', headers: { 'x-ecosystem-keys': JSON.stringify(keys) } });
  if (!res.ok) throw new Error(`Failed to trigger: ${res.status} ${await res.text()}`);
...
const triggerPlanCb = async (setIsLoading: any, setError: any, fetchTraces: any, onJobStarted?: (j: string) => void) => {
  setIsLoading(true); setError(null);
  try {
    const jobId = await doTriggerTestPlan(getEcosystemKeys());
    if (onJobStarted) onJobStarted(jobId);
    setTimeout(fetchTraces, 1000);
  } catch (err: any) { setError(err.message || 'Failed to trigger plan'); } 
  finally { setIsLoading(false); }
};
...
export function useDebug() {
  ...
  const triggerTestPlan = useCallback((cb?: any) => triggerPlanCb(t.setIsLoading, t.setError, fetchTraces, cb), [t.setIsLoading, t.setError, fetchTraces]);
  useEffect(() => { fetchTraces(); fetchQueue(); const iv = setInterval(fetchQueue, 5000); return () => clearInterval(iv); }, [fetchTraces, fetchQueue]);
  return { ...t, ...q, fetchTraces, loadMoreTraces, fetchQueue, triggerTestPlan, offsetRef };
}
