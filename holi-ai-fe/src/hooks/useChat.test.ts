...
const testAppResume = async () => {
  ...
  mockFetch.mockResolvedValue({ ok: true, json: () => Promise.resolve({ messages: [{ role: 'assistant', message: 'Resumed' }] }) });
  await act(async () => { if (resCb) await resCb({ isActive: true }); });
  await waitFor(() => expect(result.current.state.messages[0]?.content).toBe('Resumed'));
};
...
const testPendingStatusWithSystemMsg = async () => {
  ...
  const { result } = setupHook('usr_1', 'en');
  await act(async () => { await result.current.sendMessage('ping'); });
  await waitFor(() => expect(result.current.state.aiStatus).toBe('Custom pending message'), { timeout: 4000 });
};
