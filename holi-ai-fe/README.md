...
- **5-Tab Navigation**: Clean architectural separation between your telemetry Dashboard (Plan), Active Routines (Cron jobs), the AI Chat interface (Coach), BYOK configuration (Settings), and an optional LLM Debugging tab.
...
- **Dynamic Coach Personality UI**: Easily select or customize the AI Coach's strictness and conversational behavior using built-in modals. The persona defaults to "Off" (a neutral baseline) and seamlessly encapsulates your Global Unit System (Metric/Imperial) preferences.
- **Cascaded Alarms Management**: Safely tie generated background routines directly to active modules with deterministic cross-category deduplication, and clean, localized, two-step deletion confirmation UI modals. Users can fully manage (create, edit, toggle, delete) their routines via a comprehensive set of interactive modals.
...
- **Activity Reports Dashboard**: A comprehensive, interactive analytics dashboard built with `recharts`. Features dual-mode reporting: a Global Overview with vibrant Pie Charts and bar charts for category/routine distribution, and a Routine Drill-Down mode that graphs numeric progressions (e.g. weights, reps) or completion consistency over specific timeframes (7D/14D/30D/All). The chart filters elegantly synchronize with the detailed activity logs list for a seamless drill-down experience.
- **Comprehensive Vitest Suite**: Deeply abstracted custom hooks (`useChat`, `useNotifications`, `useCrons`, `useDebug`, etc.) are fully covered by a highly-mocked Vitest and React Testing Library environment ensuring flawless state and UI logic (including handling of backend `loadMore` rejections and network failures), with strict >80% branch coverage thresholds.
