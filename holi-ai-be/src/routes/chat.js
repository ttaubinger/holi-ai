const { insertJob, fetchJob, fetchPendingJob, insertEpisodicMemory, fetchEpisodicMemory, fetchUserCrons, fetchQuestionQueue } = require('../services/db');
...
const handlePendingJobGet = async (req, reply) => {
  const { userId } = req.query;
  const apiKeys = getKeys(req);
  if (!userId) return reply.status(400).send({ error: 'Missing userId' });
  const pendingJob = await fetchPendingJob(apiKeys, userId);
  return reply.send({ job: pendingJob || null });
};

const handleCronPut = async (req, reply) => {
  const { cronId } = req.params;
  const { userId, ...cronData } = req.body;
  const apiKeys = getKeys(req);
  if (!userId || !cronId) return reply.status(400).send({ error: 'Missing userId or cronId' });
  await require('../services/db').upsertCrons(apiKeys, userId, [{ ...cronData, cron_id: cronId }]);
  return reply.send({ success: true });
};
...
const chatRoutes = async (fastify) => {
  ...
  fastify.get('/chat/pending', handlePendingJobGet);
  ...
  fastify.put('/chat/crons/:cronId', handleCronPut);
