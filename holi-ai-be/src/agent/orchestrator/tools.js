...
const USER_CRON_PARAMS = { type: 'object', properties: { crons: { type: 'array', items: { type: 'object', properties: { title: { type: 'string' }, schedule: { type: 'string', description: 'A human-readable label for the schedule, e.g., "06:45 daily" or "Wed 07:00". Do NOT use raw cron notation here.' }, cron_expression: { type: 'string', description: 'Standard 5-part cron expression (e.g., "45 6 * * *").' }, category: { type: 'string', enum: ['Daily', 'Weekly', 'Monthly', 'Custom'], description: 'Frequency category of the routine.' }, description: { type: 'string', description: 'CRITICAL: Must not be vague. Must be an actionable task with a detailed description of what to do. Specificity is key so the user does not need to guess.' }, linked_module: { type: 'string' }, requires_logging: { type: 'boolean', description: 'Set to true if this routine requires the user to log a measurable activity.' }, log_type: { type: 'string', enum: ['number', 'boolean', 'text'], description: 'Type of value to log. Use number for counts, boolean for yes/no.' }, log_unit: { type: 'string', description: 'Unit for the log value. Leave empty for booleans.' } }, required: ['title', 'schedule', 'cron_expression', 'category'] } } }, required: ['crons'] };
...
const SEND_RESPONSE_TOOL_PARAMS = (language) => ({ type: 'object', properties: { headline: { type: 'string', description: 'A short dashboard headline reflecting the user\'s current coaching status.' }, diagnostic_summary: { type: 'string', description: 'A one-sentence diagnostic of the user\'s CURRENT state or baseline relative to their goals. CRITICAL: Do NOT summarize the action plan here! This must describe the USER\'S starting point.' }, chat_message: { type: 'string', description: buildChatMessageDescription(language) } }, required: ['headline', 'diagnostic_summary', 'chat_message'] });
