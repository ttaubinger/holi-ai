...
const execMocks = async (mockCreate, mockExec = jest.fn(), onStatus = jest.fn(), lang = 'en', sMsgs = null, msg = 'M', prmpt = 'P', hist = []) => {
  ...
  return executeAgentWorkflow({ groqKey: 'test-key' }, msg, prmpt, hist, lang, mockExec, [], [], false, onStatus, sMsgs);
};
...
it('should call Groq with tools and execute tool loop', async () => {
  ...
  expect(mockExec).toHaveBeenCalledWith('fetch_action_plans', {}, expect.anything());
...
it('should restrict tools to Discovery phase when primary_goal is missing', async () => {
  ...
  const Groq = require('groq-sdk');
  Groq.mockImplementation(() => ({ chat: { completions: { create: mockCreate } } }));
  await executeAgentWorkflow({ groqKey: 'test-key' }, 'Msg', 'Prompt', [], 'en', jest.fn(), [], [], false, jest.fn());
  expect(mockCreate).toHaveBeenCalledTimes(1);
...
const makePlanRoutinesMock = () => jest.fn()
  ...
  .mockResolvedValueOnce({ choices: [{ message: { content: '', tool_calls: [{ id: '2', function: { name: 'generate_user_routines', arguments: '{"plan_title":"Lose weight"}' } }] } }] })
  .mockResolvedValueOnce({ choices: [{ message: { content: '', tool_calls: [{ id: '3', function: { name: 'send_response', arguments: '{"headline":"Ready","diagnostic_summary":"","chat_message":"Your plan and routines are ready. Check the Plans tab."}' } }] } }] });
...
it('should chain generate_user_plan -> generate_user_routines and send brief response', async () => {
  ...
  const res = await executeAgentWorkflow({ groqKey: 'k' }, 'M', 'P', [], 'en', mockExec, [{ key: 'primary_goal', value: 'Lose weight' }], [], false, jest.fn());

  expect(mockExec).toHaveBeenCalledWith('generate_user_plan', expect.anything(), expect.anything());
  expect(mockExec).toHaveBeenCalledWith('generate_user_routines', expect.anything(), expect.anything());
...
it('should NOT emit plan-ready when plan_generation enqueues questions instead', async () => {
  ...
  Groq.mockImplementation(() => ({ chat: { completions: { create: mockCreate } } }));
  const res = await executeAgentWorkflow({ groqKey: 'k' }, 'M', 'P', [], 'en', jest.fn().mockResolvedValue({ success: true }), [{ key: 'primary_goal', value: 'Lose weight' }], [], false, jest.fn());
...
it('should NOT emit plan-ready when generate_user_plan tool returns error', async () => {
  ...
  Groq.mockImplementation(() => ({ chat: { completions: { create: mockCreate } } }));
  const res = await executeAgentWorkflow({ groqKey: 'k' }, 'M', 'P', [], 'en', jest.fn().mockResolvedValue({ error: 'fail' }), [{ key: 'primary_goal', value: 'Lose weight' }], [], false, jest.fn());
  expect((res.chat_message || '').toLowerCase()).not.toContain('plan is ready');
  expect((res.chat_message || '').toLowerCase()).not.toContain('plan and routines are ready');
});
