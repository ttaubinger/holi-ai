...
const extractSendResponse = (toolCalls) => {
  if (toolCalls.some(tc => ['generate_user_plan', 'generate_user_routines'].includes(tc.function.name))) return null;
...
const getToolStatusMessage = (name, lang) => {
  if (name === 'generate_user_plan') {
    return lang === 'cs'
      ? 'Generování plánu může trvat několik minut. Váš požadavek bude zpracován automaticky.'
      : 'Generating plans might take a few minutes. Your request will be processed automatically.';
  }
  if (name === 'generate_user_routines') {
    return lang === 'cs'
      ? 'Generování rutin může trvat několik minut. Váš požadavek bude zpracován automaticky.'
      : 'Generating routines might take a few minutes. Your request will be processed automatically.';
  }
...
const handleToolError = (error, toolCall, messages, functionName) => {
  ...
  if (errorMessage.includes('429') || errorMessage.includes('413') || errorMessage.includes('rate_limit') || errorMessage.includes('rate limit') || errorMessage.includes('TPM') || errorMessage.includes('JOB_WIPED')) {
    throw error;
  }
  messages.push({
    role: 'tool',
...
const processToolCall = async (toolCall, toolExecutor, messages, onStatus, lang, context) => {
  ...
  try {
    const result = await toolExecutor(functionName, functionArguments, context);
...
const filterUnexecutedCalls = (toolCalls, messages) => {
  const executedIds = new Set(messages.filter(m => m.role === 'tool').map(m => m.tool_call_id));
  const unexecuted = toolCalls.filter(tc => !executedIds.has(tc.id));
  if (unexecuted.length > 0) {
    console.log(`[AI] Tools requested (unexecuted): ${unexecuted.map(tc => tc.function.name).join(', ')}`);
  }
  return unexecuted;
};

const processAgentToolCalls = async (toolCalls, toolExecutor, messages, onStatus, lang, activeStep, context) => {
  const unexecutedCalls = filterUnexecutedCalls(toolCalls, messages);
  const sortedCalls = [...unexecutedCalls].sort((a, b) => getToolOrder(a.function.name) - getToolOrder(b.function.name));
  ...
  for (const toolCall of sortedCalls) {
    if (toolCall.function.name !== 'send_response') {
      await processToolCall(toolCall, toolExecutor, messages, onStatus, lang, context);
    }
  }
...
const executeToolCallsSafely = async (msg, toolExecutor, mainMessages, onStatus, lang, activeStep, context) => {
  try {
    const interceptResult = await processAgentToolCalls(msg.tool_calls, toolExecutor, mainMessages, onStatus, lang, activeStep, context);
    ...
    return interceptResult;
  } catch (e) { e.savedMessages = mainMessages; e.savedContext = context; throw e; }
};
...
const checkUnexecutedCalls = (lastMsg, mainMessages) => {
  if (!lastMsg || lastMsg.role !== 'assistant' || !lastMsg.tool_calls?.length) return false;
  const executedIds = new Set(mainMessages.filter(m => m.role === 'tool').map(m => m.tool_call_id));
  return lastMsg.tool_calls.some(tc => !executedIds.has(tc.id));
};
...
const processAgentTurn = async (apiKeys, mainMessages, apiMessages, toolExecutor, callbacks, context, activeStep) => {
  ...
  if (checkUnexecutedCalls(lastMsg, mainMessages)) {
    console.log(`[AI] Resuming unexecuted tool calls from previous interruption...`);
    return await handleAgentTurnOutput(lastMsg, apiMessages, toolExecutor, mainMessages, callbacks, activeStep, context);
  }
