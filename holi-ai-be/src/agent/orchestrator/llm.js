...
const checkRateLimitError = (errorMessage, attempt, maxAttempts, error, messages) => {
  ...
  const isFatalError = errorMessage.includes('400') || errorMessage.includes('failed_generation') || errorMessage.includes('invalid_request_error');
  if (isRateLimit || isFatalError || attempt === maxAttempts) {
...
const createRecoveryResponse = (toolName, args) => ({
  choices: [{
    message: {
      role: 'assistant',
      content: null,
...
const logAndRecover = (msg, toolName, data) => {
  console.log(`[AI] Recovering from Groq ${msg}`);
  return createRecoveryResponse(toolName, data);
};

const processHallucinatedPayload = (parsed, forcedTool) => {
  if (parsed.name === 'json' && parsed.arguments?.chat_message) return logAndRecover('"json" tool hallucination', 'send_response', parsed.arguments);
  if (parsed.chat_message && !forcedTool) return logAndRecover('missing-tool hallucination', 'send_response', parsed);
  if (forcedTool) {
    if (parsed.name === forcedTool && parsed.arguments) return logAndRecover(`explicit "${forcedTool}"`, forcedTool, parsed.arguments);
    return logAndRecover(`forced tool "${forcedTool}"`, forcedTool, parsed);
  }
  return null;
};

const recoverFromHallucination = (error, activeStep) => {
  const failedStr = extractFailedGeneration(error);
  if (!failedStr) return null;
  try {
    return processHallucinatedPayload(safeJsonParse(failedStr), activeStep?.force_tool_choice?.function?.name);
  } catch (_e) { /* Ignore */ }
  return null;
};
...
const createGroqCompletion = (client, model, messages, tools, tool_choice) => {
  return client.chat.completions.create(
    { messages, model, tools, tool_choice, max_tokens: 1500 },
    { timeout: 30000, maxRetries: 0 }
  );
};

const processLlmAttemptError = async (error, client, model, attempt, apiKeys, messages, onStatus, lang, onTrace, activeStep, ctx, tools) => {
  if (error.message === 'JOB_WIPED') throw error;
  const recovered = recoverFromHallucination(error, activeStep);
...
const executeAttempt = async (client, model, attempt, apiKeys, messages, onStatus, lang, onTrace, activeStep, ctx) => {
  ...
  try {
    ...
    return await processLlmAttemptError(error, client, model, attempt, apiKeys, messages, onStatus, lang, onTrace, activeStep, ctx, tools);
  }
};

const getValidModel = (model) => {
  if (!model) return 'openai/gpt-oss-120b';
  const deprecated = ['llama-3.1-8b-instant', 'llama-3.3-70b-versatile', 'qwen'];
  if (deprecated.some(d => model.toLowerCase().includes(d))) return 'openai/gpt-oss-120b';
  return model;
};

const executeAgentTurn = async (apiKeys, messages, onStatus, lang, onTrace, activeStep, context) => {
  const client = getGroqClient(apiKeys);
  const selectedModel = getValidModel(apiKeys.groqModel);
...
module.exports = { getGroqClient, executeAgentTurn, getValidModel };
