...
const updateJobStatus = async (keys, jobId, status, result) => {
  const pool = getPgPool(keys);
  if (pool) {
    const res = await pool.query('UPDATE llm_job_queue SET status = $1, result = $2 WHERE id = $3', [status, result ? JSON.stringify(result) : null, jobId]);
    if (res.rowCount === 0) throw new Error('JOB_WIPED');
    return;
  }
  const { data, error } = await getSbClient(keys).from('llm_job_queue').update({ status, result }).eq('id', jobId).select('id');
  if (error) throw new Error(error.message);
  if (!data || data.length === 0) throw new Error('JOB_WIPED');
};
...
const fetchPendingJob = async (keys, userId) => {
  const pool = getPgPool(keys);
  if (pool) {
    const res = await pool.query("SELECT * FROM llm_job_queue WHERE payload->>'userId' = $1 AND status IN ('pending', 'processing', 'delayed') ORDER BY created_at DESC LIMIT 1", [userId]);
    return res.rows[0];
  }
  const { data, error } = await getSbClient(keys).from('llm_job_queue').select('*')
    .eq('payload->>userId', userId).in('status', ['pending', 'processing', 'delayed'])
...
const wipeJobQueue = async (keys, userId) => {
  const pool = getPgPool(keys);
  if (pool) return pool.query(`DELETE FROM llm_job_queue WHERE payload->>'userId' = $1`, [userId]);
...
module.exports = { insertJob, updateJobStatus, fetchJob, fetchPendingJob, insertConfig, fetchConfig, upsertCoachPrompt, fetchCoachPrompt, wipeDatabase, insertLlmTrace, fetchLlmTraces };
