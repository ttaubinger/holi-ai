...
const isTransientMessage = (msgStr) => {
  ...
  return msgStr.includes('Reading memory') || msgStr.includes('Čtení paměti') || 
         ...
         msgStr.includes('Generating routines') || msgStr.includes('Generování rutin') ||
         msgStr.includes('Generating plans') || msgStr.includes('Generování plánu');
};
...
const matchEpisodicEmbeddings = async (keys, pool, userId, embedding, limit) => {
  const threshold = parseFloat(keys?.ragThreshold || '0.5');
  if (pool) {
