...
const getRelevantModules = async (allModules, msgEmbedding) => {
  ...
  return relevantModules.sort((a, b) => b.sim - a.sim).slice(0, 2).map(r => r.module);
};

const getRelevantCrons = async (allCrons, msgEmbedding) => {
  const relevantCrons = [];
  ...
  return relevantCrons.sort((a, b) => b.sim - a.sim).slice(0, 3).map(r => r.cron);
};
...
const buildRelevantContext = async (apiKeys, userId, message, msgEmbedding) => {
  ...
  const relevantModules = await getRelevantModules(allModules, msgEmbedding);
  const relevantCrons = await getRelevantCrons(allCrons, msgEmbedding);

  return { planIndex, cronIndex, relevantModules, relevantCrons };
};
