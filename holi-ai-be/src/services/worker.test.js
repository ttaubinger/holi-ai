...
const setupDbMock1 = () => {
  ...
  db.fetchJob = jest.fn().mockResolvedValue({ id: 'job-123' });
...
const setupDbMock2 = () => {
  ...
  db.deleteUserCron = jest.fn().mockResolvedValue(true);
...
const generateTestJob = () => {
  ...
  it('should process job, call tool executor with upsert_user_fact, and save final chat message', async () => {
    ...
    orchestrator.executeAgentWorkflow.mockImplementation(
      async (apiKeys, msg, prompt, history, lang, exec, facts, qQueue, hasPlan, onStatus, sMsgs, onTrace) => mockProcessTask(exec, onStatus, onTrace)
    );
  ...
  it('handles onTrace failure', async () => {
    ...
    orchestrator.executeAgentWorkflow.mockImplementation(async (a, b, c, d, e, f, g, q, h, i, sm, onTrace) => {
      await onTrace({ model: 'x' });
  ...
  it('should test handleFetchActionModuleDetails and onStatus coverage', async () => {
    ...
    orchestrator.executeAgentWorkflow.mockImplementation(async (apiKeys, message, prompt, history, language, executor, facts, queue, hasPlan, onStatus) => {
  ...
  it('should resume from savedMessages if present', async () => {
    ...
    expect(orchestrator.executeAgentWorkflow).toHaveBeenCalledWith(
      ...
      expect.anything(), expect.anything(), expect.anything(),
      expect.anything(), expect.anything(), expect.anything(),
      expect.anything(), [{ role: 'system', content: 'saved_state' }], expect.anything(), null
    );
  ...
  it('covers generate_user_plan', async () => {
    const createMock = jest.fn()
      .mockResolvedValueOnce({ choices: [{ message: { tool_calls: [{ function: { arguments: '{"plan_title": "p1", "description": "desc", "user_profile_summary": "profile summary", "category_names": ["cat1"]}' } }] } }] })
      .mockResolvedValueOnce({ choices: [{ message: { tool_calls: [{ function: { arguments: '{"name": "cat1", "content": "cont"}' } }] } }] });
    orchestrator.getGroqClient.mockReturnValue({ chat: { completions: { create: createMock } } });
    orchestrator.executeAgentWorkflow.mockImplementation(async (a, b, c, d, e, exec) => {
  ...
  it('covers generate_user_routines', async () => {
    const job = generateTestJob();
    database.fetchActionModules.mockResolvedValue([{ module_title: 'm1', categories: [{ name: 'cat1', content: 'test' }] }]);
