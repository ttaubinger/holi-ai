...
const dispatchToolCall2 = async (apiKeys, userId, functionName, toolArgs, jobId, context) => {
  ...
  if (functionName === 'generate_user_plan') return await handleGenerateUserPlan(apiKeys, userId, toolArgs, jobId, context);
  if (functionName === 'generate_user_routines') return await handleGenerateUserRoutines(apiKeys, userId, toolArgs, jobId, context);
  return null;
};

const handleToolCall = async (apiKeys, userId, functionName, toolArgs, history, jobId, context) => {
  const result1 = await dispatchToolCall1(apiKeys, userId, functionName, toolArgs, history);
  if (result1 !== null) return result1;
  const result2 = await dispatchToolCall2(apiKeys, userId, functionName, toolArgs, jobId, context);
