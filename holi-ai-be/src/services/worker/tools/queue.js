...
const shouldBypassRag = async (apiKeys, userId) => {
  const facts = await database.fetchUserFacts(apiKeys, userId);
  return facts.length === 0 || (facts.length === 1 && facts[0].key === 'primary_goal');
};
...
const handleEvaluateCurrentQuestion = async (apiKeys, userId) => {
  ...
  if (!(await database.fetchRagEnabled(apiKeys, userId))) await database.enableRag(apiKeys, userId);

  if (await shouldBypassRag(apiKeys, userId)) {
    return { question: queue[0], retrieved_facts: [], retrieved_memory: [] };
  }
