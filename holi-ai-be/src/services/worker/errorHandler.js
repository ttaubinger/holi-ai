...
const getRateLimitStatusOverrides = (job, existingJob) => {
  ...
  if (system_message === 'Plan generated.') {
    return { progress: 0, system_message: 'Starting routines generation...' };
  }
  if (!system_message || system_message === 'Calling tools...') {
    if (activeRoute?.includes('routine_generation')) system_message = 'Starting routines generation...';
    else if (activeRoute?.includes('plan_generation')) system_message = 'Starting plan generation...';
  }
