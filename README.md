...
4. **Dynamic Coach Personality**: Customize the strictness and behavior of your AI coach by choosing predefined personas (Strict, Empathetic, Socratic) or injecting your own custom coaching prompt. The personality feature is completely optional (defaults to "Off" for a neutral AI) and now also seamlessly embeds your preferred **Unit System** (Metric/Imperial).
5. **Dynamic Model Selection**: Connects directly to Groq to offer a real-time selection of all available open-source models (including OpenAI GPT-OSS reasoning models and DeepSeek).
...
10. **Autonomous Background Routines**: The AI autonomously formulates, consolidates, and schedules deduplicated cron jobs and alarms inside a dedicated "Routines" tab to track your progress even when you are away. Users can also manually create, edit, delete, and manage their own routines through intuitive UI modals.
...
16. **Comprehensive Activity Reports**: The frontend features a highly interactive, dual-mode analytics dashboard built with `recharts`. It offers a Global Overview (vibrant pie charts, top routine bar charts, and overall trend lines) alongside a Routine Drill-Down mode (tracking specific numeric progressions like weights and reps, or consistency patterns). The fully synchronized timeframe filters (7D/14D/30D/All) and detailed logs list give you a deep, granular dive into your AI-guided adherence.
17. **Question Queue Orchestration & RAG Purity**: Context is seamlessly injected using a dedicated Question Queue tool loop that automatically retrieves semantic facts and episodic memories related to pending conversation queries, eliminating token bloat from blind fact-finding. The queue enforces **RAG Semantic Purity** (default relevance threshold `0.5`) to retrieve only strictly relevant data without raw chat history. The Queue Router natively inspects incoming questions, instantly skipping obsolete ones and dequeuing the next valid question to immediately fetch RAG context, eliminating the need to micromanage the queue.
18. **Strictly Modular Backend Layer**: The backend features a completely decoupled PostgreSQL database schema engine (broken down logically by episodic memory, facts, crons, biometrics) and a single-responsibility worker layer, making the codebase highly maintainable and SLA-compliant.
